<script>
	 ...
	 */
	export let id = null;
	export let value = typeof initialValue === 'string' ? +initialValue : initialValue;
	...
	function onDragEnd(e) {
		...
		if (e.type === 'mouseup') {
			if (browser && document.body.contains(mouseEventShield))
	...
	/**
	 * @param {{ type?: string; clientX?: any; clientY?: any; }} event
	...
	/**
	 * @param {{ type: string; stopPropagation: () => void; preventDefault: () => void; touches: { clientX: any; }[]; clientX: any; }} e
	 */
...
<style>
	...
	.range__wrapper:focus-visible > .range__track {
		box-shadow: 0 0 0 2px white, 0 0 0 3px var(--track-focus, #cb2645);
	}

	.range__track {
		height: 6px;
		background-color: var(--track-bgcolor, #e15c77);
		border-radius: 999px;
	}

	.range__track--highlighted {
		background-color: var(--track-highlight-bgcolor, #cb2645);
		background: var(--track-highlight-bg, linear-gradient(90deg, #cb2645, #567ea6));
		width: 0;
		height: 6px;
		position: absolute;
		border-radius: 999px;
	...
	.range__tooltip {
		...
		border-radius: 4px;
		text-align: center;
		background-color: var(--tooltip-bgcolor, #955876);
		background: var(--tooltip-bg, linear-gradient(45deg, #955876, #6d7195));
	}
